...
model_face = load_model("static/data-preprocess/model/kk11CNN1.h5")
FACE_CLASSES = ['ben_afflek', 'elton_john','jerry_seinfeld','madonna','mindy_kaling']
def predict_face(image_data):
  #x = cv2.resize(image_data, (48, 48)) 
  #img = image.load_img(image_data, target_size=(48, 48))
...
@app.route('/cnn/vgg/vgg')
def vgg():
    return render_template('/cnn/vgg/vgg.html')


@app.route('/cnn/vgg/vgg',  methods=['GET', 'POST'])
def vgg1():
    ...
    if request.method == 'POST':
        ...
        return render_template('/cnn/vgg/vggoutput.html', model_name=my_model_name,my_dataset=input_image, pred=preds, visualize=input )
